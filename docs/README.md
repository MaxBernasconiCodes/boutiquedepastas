# Documentación — Boutique de pastas

Documentación del proyecto. **Mantenerla actualizada** cuando se modifiquen flujos, estructura o despliegue (rutas, stores, variables de entorno, Netlify, etc.).

## Índice

- [Arquitectura y flujos](./ARQUITECTURA.md)
- [Iconos (Material Symbols Rounded)](./ICONOS.md)
- [Despliegue en Netlify](./NETLIFY.md)
- [Netlify Forms y límites](./NETLIFY-FORMS.md)
- [**Pasos deploy y conectar Forms**](./PASOS-DEPLOY-NETLIFY-FORMS.md) — guía paso a paso en español
- [**Base de datos Neon**](./NEON-SETUP.md) — usar Neon (PostgreSQL) en lugar de Forms para el panel de administración
